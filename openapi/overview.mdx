...
<Warning>
  Lynx is currently in pilot. To join the pilot or build an integration against
  the OpenAPI, contact [partners@hvnn.me](mailto:partners@hvnn.me).
</Warning>
...
at `https://api.lynxops.co/openapi.json`. See [OpenAPI schema](/openapi/schema)
...
| `POST` | `/firewall-approvals`              | `events:write`                 | Request approval for one protected tool call.                  |
| `GET`  | `/firewall-approvals/{approvalId}` | `policies:read`                | Poll the current approval status.                              |
...
For `REQUIRE_APPROVAL`, create one request with a stable
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="user-check"
    href="/openapi/firewall-approvals"
  >
