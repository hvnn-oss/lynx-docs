...
Use `firewallInput()` before generation and `firewallOutput()` before returning
generated content. Managed policies are evaluated against the original value
locally. Set `captureContent: false` when the value may be used for policy
evaluation but must not be attached to policy events.

```ts
lynx.firewallInput(userMessage, {
  label: "chat.input",
